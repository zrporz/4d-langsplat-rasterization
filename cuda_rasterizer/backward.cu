	...
	bool include_feature)
{
	...
	//######################## DEBUG ########################
	// if(block.thread_rank()==100){
	// 	printf("Start renderCUDA\n");
	// }
	//################################################
	...
	//######################## DEBUG ########################
	// if(block.thread_rank()==100){
	// 	printf("block.thread_rank():%d, dL_dpixel_depth:%f\n",block.thread_rank(),dL_dpixel_depth);
	// }
	//################################################
	float last_alpha = 0;
	...
	// if(block.thread_rank()==100){
	// printf("start cal dL_dpixel_F\n");
	// }
	// for (int i = 0; i < F; i++)
	// 	printf("dL_dpixel_F[%d]:%f\n",i,dL_dpixel_F[i]);
	if (include_feature) {
		if (inside)
			for (int i = 0; i < F; i++)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		//######################## DEBUG ########################
		// if(block.thread_rank()==0){
		// 	printf("collected_id[block.thread_rank()]:%d\n",collected_id[block.thread_rank()]);
		// 	printf("collected_xy[block.thread_rank()]:%f,%f\n",collected_xy[block.thread_rank()].x,collected_xy[block.thread_rank()].y);
		// 	printf("collected_conic_opacity[block.thread_rank()]:%f,%f,%f,%f\n",collected_conic_opacity[block.thread_rank()].x,collected_conic_opacity[block.thread_rank()].y,collected_conic_opacity[block.thread_rank()].z,collected_conic_opacity[block.thread_rank()].w);
		// 	printf("collected_colors[block.thread_rank()]:%f\n",collected_colors[block.thread_rank()]);
		// 	printf("collected_feature[block.thread_rank()]:%f\n",collected_feature[block.thread_rank()]);
		// 	printf("collected_depths[block.thread_rank()]:%f\n",collected_depths[block.thread_rank()]);
		// }
		//################################################
